...
def plot_cartpole_performance():
    ...
    data = pd.DataFrame()
    for p in Path("data_final_invpend").glob("*"):
...
def plot_double_cartpole_performance():
    sns.set_theme("paper", "whitegrid")

    data = pd.DataFrame()
    for p in Path("data_final_dinvpend").glob("*"):
    ...
    plt.savefig("results/double_cartpole_eval.png")


def plot_double_cartpole_heatmap():
    sns.set_theme("paper", "whitegrid")
    ...
    for i, name in enumerate(["GP", "GP-E5", "GP-E10", "GP-E15", "NN"]):
        d = data[name]
        d *= 180 / np.pi
        df = pd.DataFrame(d[:, 1:3], columns=["Theta1", "Theta2"])
        sns.histplot(
            df,
            x="Theta1",
            y="Theta2",
    ...
    plt.savefig("results/double_cartpole_heatmap.png")


if __name__ == "__main__":
    # plot_cartpole_performance()
    # plot_cartpole_heatmap()
    plot_double_cartpole_performance()
    plot_double_cartpole_heatmap()
