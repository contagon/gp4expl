...
class RNDModel(nn.Module, BaseExplorationModel):
    def __init__(self, hparams, optimizer_spec, **kwargs):
        ...
        self.f = ptu.build_mlp(self.ob_dim, self.output_size, self.n_layers, self.size, activation="relu")
        init_method_1(self.f)
        self.f.to(ptu.device)

        self.f_hat = ptu.build_mlp(self.ob_dim, self.output_size, self.n_layers, self.size, activation="relu")
        init_method_2(self.f_hat)
        self.f_hat.to(ptu.device)

        self.optimizer = optim.Adam(self.f_hat.parameters(), **self.optimizer_spec)
    ...
    def forward(self, ob_no):
        ...
        f_output = self.f(ob_no)
        f_output.detach()
        f_hat_output = self.f_hat(ob_no)
        return f_hat_output - f_output
