...
class MPCPolicy(BasePolicy):
        ...
        if self.sample_strategy == 'random' \
            ...
            for i in range(self.cem_iterations):
                ...
                if not cem_mean.any():
                    ...
                    actions = np.random.normal(cem_mean, cem_std, (num_sequences, horizon, self.ac_dim))
                ...
                cem_mean = actions_best.mean(axis=0)*self.cem_alpha + (1 - self.cem_alpha)*cem_mean
                cem_std = actions_best.std(axis=0)*self.cem_alpha + (1 - self.cem_alpha)*cem_std
    ...
    def calculate_sum_of_rewards(self, obs, candidate_action_sequences, model):
        ...
        predicted_obs[:,0] = obs
        for j in range(H):
            sum_of_rewards += self.env.get_reward(predicted_obs[:, j], candidate_action_sequences[:, j])[0]
            predicted_obs[:,j+1] = model.get_prediction(predicted_obs[:, j], candidate_action_sequences[:, j], self.data_statistics)
